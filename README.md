## Olá 👋

# Felipe Mendes

📊 Data Analytics focused professional with a strong foundation in **SQL, data analysis and backend systems**.

Currently building projects using **PostgreSQL, analytical SQL and real-world datasets**, while leveraging prior experience with **Python and APIs**.

---

## 🚀 Current Focus

- Advanced SQL for data analysis  
- Data modeling and relational databases  
- Analytical thinking and metric design  
- Preparing for data visualization (Power BI)  

---

## 📂 Featured Project

### 📊 Data Analytics Lab
Repository focused on real-world data analysis using the **Olist Brazilian E-commerce Dataset**.

**Highlights:**
- Advanced SQL queries (CTEs, subqueries, window functions)
- Product and customer ranking analysis
- Revenue share and cumulative metrics (Pareto analysis)
- Monthly performance analysis (MoM)
- Customer behavior and purchase history

👉 [**View Repository**](https://github.com/Felipee-M/data-analytics-lab)

---

## 🛠 Tech Stack

### Languages
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![SQL](https://img.shields.io/badge/SQL-336791?style=for-the-badge&logo=postgresql&logoColor=white)

---

### Data & Databases
![PostgreSQL](https://img.shields.io/badge/PostgreSQL-000?style=for-the-badge&logo=postgresql)
![MySQL](https://img.shields.io/badge/MySQL-00000F?style=for-the-badge&logo=mysql&logoColor=white)

---

### Backend (Support Skills)
![Django](https://img.shields.io/badge/django-%23092E20.svg?style=for-the-badge&logo=django&logoColor=white)
![Django REST Framework](https://img.shields.io/badge/django_rest_framework-red?style=for-the-badge)

---

### Tools
![Git](https://img.shields.io/badge/GIT-E44C30?style=for-the-badge&logo=git&logoColor=white)
![VSCode](https://img.shields.io/badge/Vscode-007ACC?style=for-the-badge&logo=visual-studio-code&logoColor=white)
![HTTPie](https://img.shields.io/badge/httpie-orange?style=for-the-badge)

---

## 🌐 Connect with me

[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?style=for-the-badge&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/felipemendessantos/)
[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/Felipee-M)

---

## 📊 GitHub Stats

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=Felipee-M&theme=transparent&show_icons=true&hide_title=true)

![Top Langs](https://github-readme-stats.vercel.app/api/top-langs/?username=Felipee-M&layout=compact&theme=transparent&hide_title=true)

---

⭐ Transitioning into Data Analytics with a strong technical foundation and hands-on projects.
